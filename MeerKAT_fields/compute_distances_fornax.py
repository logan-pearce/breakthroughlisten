...
#######################################################################################################
print 'Starting with Fornax'
start=time.time()
print 'Adding rows to table...'
string = "ALTER TABLE `meerkat_gaiadr2_fornax` \
...
for x in xrange(0,numrows):
    ...
    string = """UPDATE `meerkat_gaiadr2_fornax` SET `dist.c`="""+str(d[0])+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
    string = """UPDATE `meerkat_gaiadr2_fornax` SET `fwhm_lo.c`="""+str(lo)+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
    string = """UPDATE `meerkat_gaiadr2_fornax` SET `fwhm_hi.c`="""+str(hi)+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
    string = """UPDATE `meerkat_gaiadr2_fornax` SET `sd.c`="""+str(sigma)+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
end=time.time()
