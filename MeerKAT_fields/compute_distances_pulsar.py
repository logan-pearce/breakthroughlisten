...
#######################################################################################################
print 'Starting with Pulsar Timing'
start=time.time()
print 'Adding rows to table...'
string = "ALTER TABLE `meerkat_gaiadr2_pulsar_timing` \
...
c.execute("""SELECT source_id,parallax,parallax_error \
                FROM meerkat_gaiadr2_pulsar_timing\
                WHERE 1 """)
numrows = c.rowcount

print 'Starting computation! Hold on to your butts!'
...
for x in xrange(0,numrows):
    ...
    string = """UPDATE `meerkat_gaiadr2_pulsar_timing` SET `dist.c`="""+str(d[0])+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
    string = """UPDATE `meerkat_gaiadr2_pulsar_timing` SET `fwhm_lo.c`="""+str(lo)+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
    string = """UPDATE `meerkat_gaiadr2_pulsar_timing` SET `fwhm_hi.c`="""+str(hi)+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
    string = """UPDATE `meerkat_gaiadr2_pulsar_timing` SET `sd.c`="""+str(sigma)+""" \
                    WHERE `source_id` ="""+str(row[0])
    cursor.execute(string)
end=time.time()
