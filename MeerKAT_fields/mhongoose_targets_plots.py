...
f = pd.read_sql("SELECT source_id,parallax,bp_rp,phot_g_mean_mag,ra,decl FROM meerkat_gaiadr2_mhongoose\
                WHERE 1", con=db)
...
ax.set_title("CMD of Gaia sources in within Mhongoose field")
ax.set_ylabel('M$_{G}$')
...
plt.savefig('mhongoose_cmd.png',format='png')
plt.close(fig)
...
ax.set_title('Stars within Mhongoose field \n')
cb = fig2.colorbar(hb, ax=ax,orientation="horizontal", pad=0.1)
cb.set_label('log10(count)')
ax.grid(True)
plt.savefig('mhongoose_mollweide.png',format='png')
plt.close(fig2)
